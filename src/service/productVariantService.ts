...
export const productVariantService = {
  ...
  async createVariant(input: CreateVariantInput): Promise<ProductVariant> {
    ...
    if (!product.hasVariants) {
      throw new CustomError(
        "Product does not support variants. Set hasVariants to true first.",
        400,
      );
    }
    ...
    const sku =
      input.sku ||
      (await this.generateSKU(input.productId, input.color, input.size));
    ...
    const variantData: any = {
      ...input,
      sku,
      variantImages: input.variantImages
  ...
  async updateVariant(
    ...
    if (input.color !== undefined || input.size !== undefined) {
      const color =
        input.color !== undefined ? input.color : existingVariant.color;
